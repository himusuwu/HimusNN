...
void Progress::update(int epoch, int epochs, double last_mse, double last_acc, double val_mse, double val_acc, std::chrono::duration<double> last_eta_s, std::chrono::duration<double, std::milli> last_epoch_ms)
{
	...
	if(use_color)
	{
		...
		const char* val_acc_color =
		(val_acc < 0.7) ? CLR_RED :
		(val_acc < 0.9) ? CLR_YEL : CLR_GRN;

		const char* val_mse_color =
		(val_mse < 1e-3) ? CLR_GRN :
		(val_mse < 1e-2) ? CLR_YEL : CLR_RED;
		...
		std::cout << std::fixed << std::setprecision(6)
			...
			<< "val_mse=" << val_mse_color << val_mse << CLR_RST << " "
			<< std::setprecision(3)
			<< "val_acc=" << val_acc_color << val_acc << CLR_RST << " "
			<< std::setprecision(2)
